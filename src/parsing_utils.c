...
static cl_float     cjGetArrFloat(cJSON *array, cl_int index)
{
...
void	save_float3(cl_float3 *target, cJSON *cj_float3_arr, float min, float max)
{
    if (cj_float3_arr)
    {
        if (cJSON_GetArraySize(cj_float3_arr) != 3)
            parsing_error("[-] Error Parsing Float3", cj_float3_arr->string);

        if (!cJSON_IsNumber(cJSON_GetArrayItem(cj_float3_arr, 0))
            || !cJSON_IsNumber(cJSON_GetArrayItem(cj_float3_arr, 1))
            || !cJSON_IsNumber(cJSON_GetArrayItem(cj_float3_arr, 2)))
            parsing_error("[-] Error Parsing Float3", cj_float3_arr->string);
        (*target).x = (cl_float)minmax_float(cjGetArrFloat(cj_float3_arr, 0), min, max);
        (*target).y = (cl_float)minmax_float(cjGetArrFloat(cj_float3_arr, 1), min, max);
        (*target).z = (cl_float)minmax_float(cjGetArrFloat(cj_float3_arr, 2), min, max);
    }
...
void	save_float(cl_float *target, cJSON *float_obj, float min, float max)
{
	...
	{
		if (!cJSON_IsNumber(float_obj))
			parsing_error("[-] Error Parsing Float", float_obj->string);
