...
float intersect_object(const t_object *sphere, const t_ray *ray);
bool intersect_scene(__constant t_object *spheres, const t_ray *ray, float *t, int *sphere_id, const int sphere_count);
...
t_ray create_cam_ray(float3 camera_pos, const int x_coord, const int y_coord, const int width, const int height)
{
	...
	return ray;
				...
				/* (__global t_object* sphere, const t_ray* ray) */
float intersect_object(const t_object *sphere, const t_ray *ray) /* version using local copy of sphere */
{
...
bool intersect_scene(__constant t_object *spheres, const t_ray *ray, float *t, int *sphere_id, const int sphere_count)
{
	...
	*t = inf;
	...
	for (int i = 0; i < sphere_count; i++)
	{
		...
		t_object sphere = spheres[i];
		
		/* float hitdistance = intersect_object(&spheres[i], ray); */
		float hitdistance = intersect_object(&sphere, ray);
...
float3 trace_path(__constant t_object *spheres, const t_ray *camray, const int sphere_count, const int *seed0, const int *seed1)
{
	...
	for (int bounces = 0; bounces < NUM_BOUNCES; bounces++)
	{
		...
		t_object hitsphere = spheres[hitsphere_id]; /* version with local copy of sphere */
...
float3 get_direct_light(__constant t_object *spheres, const t_ray *camray, const int sphere_count,
						__constant t_light *lights, const int num_lights)
{
	...
	for (int i = 0; i < num_lights; i++)
	{
		...
		if (intersect_scene(spheres, &ray, &t, &hitsphere_id, sphere_count) && t < dist_to_light)
		{
			...
			// diffuse += intensity * lights[i].emission * hitsphere.color;
			...
			// specular += lights[i].emission * pow(phong_term, hitsphere.specular_exp);
		...
		if (intersect_scene(spheres, &refl_ray, &t, &hitsphere_id, sphere_count))
			hit_color += spheres[hitsphere_id].color * 0.4f;
	}

 	hit_color += hitsphere.diffuse * diffuse + hitsphere.specular * specular;

	return hit_color;
}

__kernel void main(__constant t_object *spheres, const int sphere_count,
							__constant t_light *lights, const int num_lights,
							__global float3 *output, const int width, const int height)
{
	...
	float3 camera_pos = (float3)(0.0f, 0.2f, 2.0f);
	...
	float3 finalcolor = get_direct_light(spheres, &camray, sphere_count, lights, num_lights);
